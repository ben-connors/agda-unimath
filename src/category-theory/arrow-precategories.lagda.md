...
The
{{#concept "arrow precategory" Disambiguation="of a precategory" Agda=arrow-Precategory}}
of a [precategory](category-theory.precategories.md) `C` is the precategory with
objects the total hom types of `C` (i.e. triples `(x : C, y : C, f : x → y)`)
and morphisms `(x, y, f) → (x', y', g)` pairs of morphisms in `C`
`(u : x → x', v : y → y')` giving commutative squares in `C`.
...
Alternatively, we can define the arrow precategory to be the
[precategory of functors](category-theory.precategory-of-functors.md) from the
[representing arrow category](category-theory.representing-arrow-category.md)
into `C`.
