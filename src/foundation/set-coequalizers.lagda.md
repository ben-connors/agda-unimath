...
Any pair of maps between sets `f, g : A → B` has a coequalizer `B/A` given by
the quotient of the set `B` by the relation generated by `x ~ y` if there is
some `z : A` with `f(z) = x` and `g(z) = y`.
